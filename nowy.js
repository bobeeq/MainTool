...
Map.prototype.add = function(key, val) {
    ...
    if(current) {
        if( ! Array.isArray(current)) throw new Error('Powinna byÊ tablica');
        current.push(val);
    } else {
        this.set(key, [val]);
    }
...
class App {
    ...
    comingBasketsUrlBtn(model) {
        ...
        if (!allowed) return null;
        return btn({ value: 'Jadπce koszyki', url: arrivingBasketsUrl(model) });
    }
    ...
    async productListBox() {
        ...
        const productListBox = box('Lista produktÛw');
        const btnsContainer = html('div', { classes: 'tomczuk-product-btns-container tomczuk-row-btns' });
...
class NativeCtrl {
    ...
    /** @DONE
     * Nadpisuje bazowπ konfiguracjÍ z Native konfiguracjπ z Controllera.
     * @see basicCfg
     ...
     */
    runListsObserver() {
        var mutationSecondCheck = false;    // @DEBUG do debugowania, przed produkcjπ wyjebaÊ.
        let longestGap = 0;
        let observer = new MutationObserver(entries => {
            ...
            if(mutationSecondCheck) log('JESZCZE COå TAM PYK£O!'); // @DEBUG
            // secFromStart();      // @DEBUG
            // log(entries);        // @DEBUG
        });
    ...
    /** @TODO - metoda bÍdzie odpalana po za≥adowaniu boxÛw produktowych w DOMie
    ...
    /** @DONE
     * @THINK: czy na pewno NativeCtrl? Moøe gdzieú to przenieúÊ?
     */
...
class Controller {
    ...
    /** @THINK
     * bÍdzie zmiana referencji do listy produktÛw.
     */
    async salesReportForLoadedBoxes(duration = 2, delay = 0) {
        ...
        let report = [{
            tytul: "Øabki grajπ w ≥apki. Maluszki Êwiczπ rπczki, nÛøki i paluszki",
            model: "9788382404036",
            ean: "9788382404036",
            ...
            tytul: "NajszczÍúliwsza ksiπøka pod chmurkπ",
            model: "9788382511468",
            ean: "9788382511468",
            ilosc_zamowionych: "251",
            ilosc_unikalnych_zamowien: "227",
...
function toPrice(price) {
    ...
    return parseFloat(stripped).toFixed(2).replace('.',',') + 'z≥';
}
...
/** @THINK - moøe pomyúleÊ o stylach w konsoli?
 * 
...
function noPx(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (!regexed) throw new Error('B≥Ídny argument dla funkcji noPx');
...
function userSelection(strong = false, exceptions = '') {
    ...
    if (strong) result = result.replace(new RegExp('[^a-z0-9ÍÛπú≥øüÊÒ' + exceptions + ']', 'ig'), ' ');
...
async function getSaleReportForProduct(model = null, duration = null, delay = null) {
    ...
    if( ! report) {
        box.innerText = 'Nie mogÍ pobraÊ raportu.';
        return null;
    };
	box.innerHTML = '';
    ...
    box.innerHTML += "Sprzedaø dzienna: <strong>" + String(parseFloat(parseInt(report.ilosc_zamowionych) / duration).toFixed(1)).replace('.',',') + '</strong><br>';

    box.innerHTML += 'år. cena sprz: <strong>' + String((parseFloat(report.wartosc_produktow) / parseFloat(report.ilosc_zamowionych)).toFixed(2)).replace('.',',') + '</strong> z≥<br>';

    box.innerHTML += 'Stan: <strong>' + report.na_mag_i_zapas_z_kolejka + '</strong><br>';
    //@todo: sprawdziÊ czy na_mag_i_zapas czy na_mag_i_zapas_z_kolejka
